...
$(async function(){
    ...
    $.get({url: `http://api.curly.team/twitch/?user=${streamers_arr[i]}`, async: false, success: function(result){
    let data;
    if (result.length > 0) {
...
$.get({url: `http://api.curly.team/trovo/?user=mikamore_`, async: false, success: function(result){
    let data;
